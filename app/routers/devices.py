...
@router.post("/", response_model=DeviceResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_max_permission_level(3))])
...
@router.get("/", response_model=List[DeviceResponse], dependencies=[Depends(require_max_permission_level(4))])
def read_clients(
    q: Optional[str] = None, 
    skip: int = 0, 
    limit: int = 20, 
...
@router.patch("/{device_id}", response_model=DeviceResponse, dependencies=[Depends(require_max_permission_level(3))])
...
@router.patch("/{device_id}/transfer/{mew_client_id}", dependencies=[Depends(require_max_permission_level(2))])
